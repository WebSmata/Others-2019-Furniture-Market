<?php
	...
	define( "DB_DSN", "mysql:host=localhost;dbname=market" );
	define( "DB_USER", "root" );
	define( "DB_PASS", ""  );
	define( "CORE", "core/" );
	define( "PAGES", "pages/" );
	...
	define( "SITENAME", "Online Furniture Market"  );
	
	require( CORE . "order.php" );
	require( CORE . "user.php" );
	require( CORE . "furniture.php" );
	...
	if (!as_check_db_value('userid', 'level', 5, 'users'))  {
		$as_err = array();
		$as_err['errno'] = 2;
		$as_err['errtitle'] = 'Create Your Own Account First';
		$as_err['errsumm'] = 'There are no users yet! That means you need to set up your own account to proceed';
		require_once CORE .  'error.php';
		exit(); 
	}